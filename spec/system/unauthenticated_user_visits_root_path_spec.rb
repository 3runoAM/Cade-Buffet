...
describe 'Unauthenticated user visits root_path' do
  ...
  it 'and sees event details' do
    ...
    second_event = Event.create!(name: 'Event 2', description: 'Description 2', buffet: second_buffet,
                                 min_guests: 20, max_guests: 200, standard_duration: 600, menu: 'Menu 2',
                                 offsite_event: false, offers_alcohol: true, offers_decoration: true,
                                 offers_valet_parking: true)
    ...
    expect(page).not_to have_content 'Buffet 2'
    expect(page).not_to have_content 'Descrição: Description 2'
    expect(page).not_to have_content 'CNPJ: 18.312.384/0001-43'
    expect(page).not_to have_content 'Telefone: 222-222-2222'
    expect(page).not_to have_content 'buffet2@example.com'
    expect(page).not_to have_content 'Street 2, 2, Neighborhood 2, City 2 - State 2, 22222'
    expect(page).not_to have_content 'Event 2'
    expect(page).not_to have_content 'Description 2'
    expect(page).not_to have_content 'Número de convidados: de 20 a 200'
    expect(page).not_to have_content 'Duração do evento: 6h'
    expect(page).not_to have_content 'Cardápio: Menu 2'
    expect(page).not_to have_content 'Evento externo: Não'
    expect(page).not_to have_content 'Preço mínimo: R$ 2.000,00'
    expect(page).not_to have_content 'Adicional por convidado extra: R$ 200,00'
    expect(page).not_to have_content 'Adicional por hora extra: R$ 100'
    expect(page).not_to have_content 'Preço mínimo: R$ 2.500,00'
    expect(page).not_to have_content 'Adicional por convidado extra: R$ 250,00'
    expect(page).not_to have_content 'Adicional por hora extra: R$ 125'
  end
end